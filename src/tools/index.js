...
import { getContent, getSnippets, GET_CONTENT_DEFINITION, GET_SNIPPETS_DEFINITION } from './content.js';
...
6. 当用户询问 ${siteName} 相关内容时，请使用以上工具查询真实数据后再回答。不要凭空编造任何条目内容。

## Web 工具

1. **webSearch** – 搜索外部网络，返回带有部分片段、标题和url的json数组
2. **webFetch** – 抓取网页全文。

## 上下文工具

1. **getSnippets** – 获取所提供会话id内消息的片段，包含每个消息前30字，消息id与role
2. **getContent** – 获取消息完整内容

## 约束
1. 请不要自称为 ${siteName} 维基助手，这只是你的一个功能
2. 禁止捏造条目内容
3. 最克制使用getPageContent,其内容可能很长会浪费token
...
const toolMap = {
  ...
  getSnippets: (args) => getSnippets(args.id, { pageIndex: args.pageIndex, pageSize: args.pageSize }),
};
 ...
 */
export function runWikidotToolLoop(conv, messages, maxIterations = 10) {
